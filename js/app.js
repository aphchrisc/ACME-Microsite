...
const Visualizations = {
  ...
  renderShareOfVoice() {
    ...
    const chartData = [
      {
        ...
        x: [
          data.organizational_staff.count,
          data.community_members_patrons.count,
          data.creatives.count,
        ],
        type: "bar",
        orientation: "h",
        text: [
        ...
        hovertemplate:
          "<b>%{y}</b><br>Count: %{x}<br>Percentage: %{text}<extra></extra>",
      },
    ];
    ...
    const layout = {
      ...
      xaxis: {
        ...
        showgrid: false,
        range: [0, 950], // Increased range to ensure labels are visible
      },
      yaxis: {
        showgrid: false,
      ...
      margin: {
        t: 40,
        r: isMobile ? 80 : 120, // Increased right margin for labels
        b: 60,
  ...
  renderThemesPieChart() {
    ...
    // Clear container first to prevent overlap
    container.innerHTML = "";

    if (isMobile) {
      // Mark container as mobile bar mode so CSS can allow auto height
      container.classList.add("mobile-themes-bar");
      ...
      container.innerHTML = `
        <div class="themes-bar-chart" style="margin-bottom: 80px;">
          <h3>Top 10 Themes</h3>
          <div class="themes-chart-container">
            ...
            `
              ...
              .join("")}
            <div class="chart-footer" style="margin-top: 20px; margin-bottom: 40px;">
              <span>Percentage of Total Comments</span>
            </div>
          </div>
        </div>
      `;
      ...
      // Ensure mobile-only class is removed on desktop/donut mode
      container.classList.remove("mobile-themes-bar");
      // Desktop pie chart
      // Create labels with percentages
      const labelsWithPercent = data.labels.map(
        (label, i) => `${label}<br>${data.values[i]}%`
      );
      ...
      const chartData = [
        {
          ...
          hovertemplate:
            "<b>%{label}</b><br>%{value}% of responses<extra></extra>",
          hoverlabel: {
            bgcolor: "white",
            bordercolor: "#e2e8f0",
            font: { color: "#2d3748" },
    ...
    const chartData = [
      {
        ...
        marker: {
          color: barriers.map(
            (_, i) => ChartConfig.colors[Object.keys(ChartConfig.colors)[i]]
          ),
        },
  ...
  renderMaps() {
    ...
    if (transportMap) {
      transportMap.innerHTML =
        '<div class="map-placeholder">Transportation barriers map<br><small>Showing areas with highest impact</small></div>';
    }
  },
  ...
  renderZipCodeMap() {
    ...
    const maxResponses = Math.max(
      ...Object.values(zipcodeData).map((d) => d.responses)
    );
    ...
    Object.entries(zipcodeData).forEach(([zipcode, data]) => {
      ...
      if (data.responses > 30) {
        // Only show labels for zip codes with significant responses
    ...
    ACME_DATA.themes.forEach((theme, index) => {
      ...
      card.innerHTML = `
        ...
        <div class="theme-info">
          <h4>${theme.name}</h4>
          <div class="theme-bar">
            <div class="theme-bar-fill" style="width: ${
              theme.percentage
            }%; background-color: ${
        ChartConfig.colors[Object.keys(ChartConfig.colors)[index % 7]]
      }"></div>
          </div>
...
window.addEventListener("resize", () => {
  ...
  resizeTimer = setTimeout(() => {
    ...
    if (wasMobile !== isMobile) {
      // Clear and re-render the themes chart properly
      const container = document.getElementById("themes-pie-chart");
      if (container) {
        // Destroy any existing Plotly chart
        if (
          container.classList &&
    ...
    // Redraw all Plotly charts on resize (but skip themes chart as we handle it above)
    const plotlyCharts = document.querySelectorAll(".js-plotly-plot");
    plotlyCharts.forEach((chart) => {
      if (chart.id !== "themes-pie-chart") {
